...
def loadDataSet():
    ...
    classVec = [0,1,0,1,0,1]#class/label
...
def trainNB0(trainMatrix, trainCategory):
    numTrainDocs = len(trainMatrix)#共有多少文本
    numWords = len(trainMatrix[0])#词库长度
    pAbusive = sum(trainCategory)/float(numTrainDocs)#P(abusive)=p(class=1)
    # p0Num = zeros(numWords); p1Num = zeros(numWords)
    # p0Denom = 0.0; p1Denom = 0.0
    p0Num = ones(numWords);   # 避免一个概率值为0,最后的乘积也为0
    p1Num = ones(numWords);   # 用来统计两类数据中，各词的词频
    p0Denom = 2.0;  # 用于统计0类中的词的总数
    p1Denom = 2.0  # 用于统计1类中的总数
    for i in range(numTrainDocs):
        if trainCategory[i] == 1:
            #print(trainMatrix[i])
            p1Num += trainMatrix[i]#都是长度为numwords的序列
            p1Denom += sum(trainMatrix[i])#分类为1的词量总数
        else:
            p0Num += trainMatrix[i]
    ...
    print(p1Num,p1Denom)
    #p1Vect 返回的是长度为numwords的每个词的概率 p(wordi|class=1)
